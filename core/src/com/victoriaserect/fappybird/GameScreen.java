...
public class GameScreen implements Screen {
    ...
    public GameScreen(final Game aGame) {
		...
		user = localDataHandler.getUserData();
    ...
    public void render(float delta) {
		...
		if (gameState == 1 && (birdY > 0 || velocity < 0)) {
			...
			if (tubeX < (birdX - bottomtube.getWidth() - birds[birdVariant][0].getWidth() / 2) && !actualTubeSucceed) {
				...
				if (score > user.getHighestScore()) {
					Label newHighScore = new Label("NEW HIGH SCORE!", rainbowFontSkin);
					newHighScore.setFontScale(3);
					newHighScore.setAlignment(Align.center);
					newHighScore.setX((Gdx.graphics.getWidth() - newHighScore.getWidth())/2);
					newHighScore.setY(Gdx.graphics.getHeight() - newHighScore.getHeight()*3);
		...
		if (gameState == 2) {
			...
			//save gamedata to the local storage
			if (!actualGameSaved) {
				localDataHandler.addOneGame(gameStarted, score);
				actualGameSaved = true;
			}
			...
			if (Gdx.input.justTouched()) {
				...
				ImageButton home = new ImageButton(gameSkin);
				home.setWidth(Gdx.graphics.getWidth()/2);
				home.setHeight(home.getWidth()/4);
				home.setPosition((Gdx.graphics.getWidth()-home.getWidth())/2, Gdx.graphics.getHeight()-playAgainButton.getHeight());
				home.getStyle().imageUp = new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("home.png"))));
				home.getStyle().imageDown = new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("home.png"))));
